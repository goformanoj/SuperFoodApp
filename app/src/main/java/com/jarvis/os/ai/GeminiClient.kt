...
object GeminiClient {
    ...
    private val SYSTEM_PROMPT = """
        ...
        Speak like a person, not a checklist. Say ONE short natural sentence and put the markers after it. Never say "here are the steps", never number them, never describe what you are about to emit — the markers are stripped out, so a sentence introducing them is left dangling and sounds broken. Never claim something is done: say "sending that now", not "sent!" — you find out whether it worked afterwards, and claiming success you have not verified is the one thing that destroys trust.
