...
> **Wake-word load — CAUGHT in CI then FIXED by reimplementing the melspectrogram in Kotlin (no device):** the emulator instrumented job caught the `CONV_2D ... BytesRequired overflowed` load crash on **every** Android runtime tried — `tensorflow-lite` 2.16.1/2.17.0 (frozen; 2.17.0 is Maven Central's last) **and** LiteRT 1.0.0/2.1.0 — while the Python `ai-edge-litert` runtime loads the same model fine. The bug is Android-runtime shape-inference on the melspectrogram model's **dynamic-length input**, not fixable by a version bump. Fix: the melspectrogram (a standard librosa power-to-db mel) is now computed in **pure Kotlin** (`voice/MelSpectrogram.kt`) using the model's **own extracted weights** (`assets/openwakeword/melspec_weights.bin`, generated by `scripts/owwtest/extract_melspec_weights.py`); the two fixed-shape feature models stay on the standard `tensorflow-lite:2.17.0`. Verified in Python (matches the model to ~1.5e-5; identical end-to-end detection 0.998/0.000) before pushing. `OpenWakeWord.lastLoadError` now names any load failure's real cause in CI. Emulator load+detect verifies in CI — build pending.
