...
> **Rigorous Parts A/B/C/F scenario suite (CI pending):** ~50 new tests over the pure seams — `ScreenControlScenarioTest` (Part A multi-step + Part C long errands via the guard chain + `AgentLoop.parseMove` driver), `ScreenMatchAccuracyTest` (Part C "tap the right control" + redaction), `WorkSessionScenarioTest` (Part B one long mic-owner lifecycle + phrase tables), and Part F (`ArtifactStoreRobolectricTest`, `ArtifactActionsScenarioTest`, emulator `ArtifactWriterInstrumentedTest` proving a real %PDF is written). Honest scope: Part F's flow-chart/diagram + image generation are NOT built, so not testable; the text/PDF path is covered. Device-only ceiling (Rule 5) unchanged: real taps on real app layouts, the LLM's PICK choice.
