...
> **This branch (build pending CI, not device-confirmed):** speech now keeps the recogniser's n-best alternatives and hands the near misses to the model to recover a mis-heard word ("peak" heard as "pic"); the custom-instructions screen was rebuilt; and a translucent "JARVIS is controlling the screen" tint now shows while it drives the screen (accessibility overlay, no new permission). Wake word is a pending decision — see below. See the first sections below and the 2026-08-06 memory entries.
...
- **Wake word — decision pending, NOT built.** The docs already settled this twice (2026-07-26): a `SpeechRecognizer` wake word can't work (two recognisers → `RECOGNIZER_BUSY`, reverted). A real one needs a dedicated hotword engine (Porcupine / openWakeWord / Vosk) with the Google recogniser started only after the hotword fires, respecting the one-mic-owner rule. Porcupine has a built-in "Jarvis" keyword but needs a Picovoice AccessKey; openWakeWord needs no key but more work / cleaner commercially. Put to the user to choose the engine before building.
