...
- **True mic-off wake word — Porcupine (opt-in, fallback-safe).** The upgrade the user chose. Porcupine runs a tiny on-device model on the built-in "Jarvis" keyword — the `SpeechRecognizer` is OFF while it listens, so nothing is transcribed. `HotwordController` arms/disarms only; the engine's single-owner `applyMicOwner` decides when: asleep+key → recogniser off, Porcupine holds the mic; on "Jarvis" → disarm, "Yes?", command via the recogniser; back to sleep → re-arm. Key rides the `GROQ_API_KEY` pattern (`BuildConfig.PICOVOICE_ACCESS_KEY`, empty when unset → in-app gate; passed through `build.yml`). **No key or any init failure = silent fallback to the in-app gate, so CI/app never break.** Scope: foreground+asleep this cut; background-anywhere (Porcupine in a mic foreground service — now low-risk, no second recogniser) is the next step. **User owes:** a free Picovoice AccessKey as a `PICOVOICE_ACCESS_KEY` GitHub secret. Device-unconfirmed — the mic handoff is the fragile part (Rule 5); trace lines "hotword armed / heard Jarvis / disarmed" make it legible.
- **Wake word — BUILT (in-app asleep/awake).** Rebuilt the documented in-app model without repeating either 2026-07-26 failure: no second recogniser (the `RECOGNIZER_BUSY` cause), the "Yes?" ack reuses the ordinary speak→listen path (the partial-only cause), and no custom silence hints (the never-finalised cause). `voice/WakeWord` (pure Kotlin, JUnit-tested) matches "Hey JARVIS" — tolerant of mishears (jervis/javis/travis/"service") and a leading filler, anchored to the start so "thank you Jarvis" stays the stop phrase — and splits off any trailing command. Engine `awake` flag: asleep it ignores all non-wake speech; on wake it says "Yes?" or runs the command, stays awake 18s for follow-ups, then sleeps. A work session counts as engaged. **Tap-to-wake:** the orb wakes it while asleep, no voice needed. **Honest limit:** stock Android keeps the recogniser running while asleep — it just won't act until the wake word. A true mic-off hotword still needs a DSP engine (Porcupine's built-in "Jarvis" keyword + a Picovoice AccessKey, or openWakeWord key-free) — that's the upgrade path. Device-unconfirmed (Rule 5).
