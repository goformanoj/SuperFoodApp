...
> Snapshot: session branch `claude/project-onboarding-lcnvi7` · `main` @ `37fb8c0` · last green build **#190** (artifact `jarvis-debug-apk`, 18.99 MB, confirmed) · updated 2026-08-09
> **Wake-word load — CAUGHT then FIXED in CI (no device):** the emulator instrumented job (run #208, `d12d015`) went red on `models_load_on_the_real_android_runtime` — the `CONV_2D ... BytesRequired overflowed` crash — proving the `tensorflow-lite:2.17.0` bump did **not** fix the load (that runtime line is frozen; 2.17.0 is Maven Central's last). Fix on this branch: swapped to the maintained **LiteRT** runtime — a drop-in for the Interpreter API (`org.tensorflow.lite.Interpreter` unchanged, so `OpenWakeWord.kt` is untouched), the same runtime that loads the model cleanly in the Python check. LiteRT **1.0.0** (Dec-2024, first Android release) still overflowed on the emulator (run #209 — it predates the allocator fix), so bumped to **`litert:2.1.0`** (the 2.x line matching the working Python `ai-edge-litert` 2.1.x). Also added `OpenWakeWord.lastLoadError`, surfaced in the emulator test, so a future load failure names its real cause in CI instead of a generic message. Emulator load test verifies in CI — build pending.
