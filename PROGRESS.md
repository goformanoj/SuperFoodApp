...
> **Wake-word load — CAUGHT then FIXED in CI (no device):** the emulator instrumented job (run #208, `d12d015`) went red on `models_load_on_the_real_android_runtime` — the `CONV_2D ... BytesRequired overflowed` crash — proving the `tensorflow-lite:2.17.0` bump did **not** fix the load (that runtime line is frozen; 2.17.0 is Maven Central's last). Fix on this branch: swapped to the maintained **LiteRT** runtime (`com.google.ai.edge.litert:litert:1.0.0`) — a drop-in for the Interpreter API (`org.tensorflow.lite.Interpreter` unchanged, so `OpenWakeWord.kt` is untouched), the same runtime that loads the model cleanly in the Python check. The emulator load test will verify the fix in CI. Build pending.
