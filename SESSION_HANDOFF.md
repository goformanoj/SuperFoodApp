...
- **Awaiting CI:** `bdc3489` (the theme redraw). Compilation is the only mechanical check that matters here — **how it looks and whether it holds framerate cannot be verified in this environment** and are the user's to judge.
...
- **When the brief is a set of images, ask "what is in the image that is not in mine?"** The first theme pass failed by being *generic*, not wrong — every element existed somewhere in the references, but the specific texture (fine dotted light strips, wireframe globes, lens flares, the wordmark) was missing, and texture is what makes art look like itself. "Does mine look good" is the wrong question.
