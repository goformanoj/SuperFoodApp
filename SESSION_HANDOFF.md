...
- **`main` @ `a7a19ee`** (green build **#174**, artifact confirmed) — everything is merged, including the three device-session guards (`AlarmGuard`, `AskGuard`, alarm-volume check) and the Settings lag fix.
...
- **Awaiting CI:** `4ef0a7f` — the **playbook**: sequences that run clean are remembered and replayed without calling the model. Slot templating (`play * on youtube`) is what makes it fire on more than an identical sentence. Routes doing anything irreversible are never learned, because replay skips the model and acts on a fuzzy match.
- **Next, and the biggest one: an agent loop.** The user asked whether JARVIS can do tasks it was never built for ("open Blinkit and add some items"). Nothing about Blinkit is the obstacle — the marker protocol is generic and `<<PICK>>` removes the need to know a layout. The obstacle is the SHAPE of the loop: it emits a whole plan up front from the screen it can see now, so any step past the first is planned against a screen that does not exist yet. Every failure in the traces is step 2 or later. Recovery is a patch on this, not a fix — capped at two, re-planning the whole sequence, and once emitting `Open(app=Open)`. The fix is to emit ONE action, observe the new screen, then decide the next, with the goal held across steps and a step budget. That subsumes recovery (a failed step is just a new observation) and pairs with the playbook: the loop discovers a route the hard way once, the playbook makes every repeat instant.
