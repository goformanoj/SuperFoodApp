...
- **`main` @ `49a5128`** (green build **#152**, artifact confirmed) — everything is merged: learned memory, the navigation restructure, the rate-limit and retired-model fixes, Files, step recovery, the narration fix, the shared/slimmed system prompt, and the red-build fix.
...
- **Awaiting CI:** `cee0dbf` — the artwork's own rings rotate in clipped concentric bands, and the wordmark is drawn in code again, centred. Compilation is the only mechanical check that matters here; **how it looks, whether the bands shear at their seams, and whether it holds framerate cannot be verified in this environment** and are the user's to judge.
