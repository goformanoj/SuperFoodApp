...
- **BRANCH IS RED and the cause is unknown.** Builds #163–#167 failed on a Kotlin compile error in the `AlarmGuard`/`AskGuard`/`AlarmVolume` work. `main` never left green at `31f3b05`, so nothing is at risk — but do not stack more work on the branch. `3d41cf8` removes `--stacktrace` from CI so the next log shows the actual `e:` lines; read those first.
...
- **Do not name a cause you have not seen.** The bare `$` in `AlarmGuard` was a genuine hazard and was reported to the user as the fix; build #166 proved it was not the cause. A plausible defect found while searching is not evidence that it is THE defect — the same mistake as trusting a lagging job status. Read the error, then fix.
