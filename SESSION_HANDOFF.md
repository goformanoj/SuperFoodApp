...
- **`main` @ `6b57cb7`** (green build **#177**, artifact confirmed) — everything is merged, including the three device-session guards (`AlarmGuard`, `AskGuard`, alarm-volume check), the Settings lag fix, and the **playbook**.
...
- **Awaiting CI:** `0d51d40` — the **agent loop** (`1816508`) and its compile fix. Built; see below. Builds #179–#180 were red on `compileDebugKotlin` because `GeminiClient.generate()` gained a `systemOverride` parameter while the line reading it lived in `buildPayload`, a private helper two calls down. `0d51d40` threads it through `generate → requestModel → buildPayload` — the shape `GroqClient` already used.
