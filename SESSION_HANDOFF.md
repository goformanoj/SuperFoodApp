...
- **`main` @ `af87cb9`** (green build **#155**, artifact confirmed) — everything is merged: learned memory, the navigation restructure, the rate-limit and retired-model fixes, Files, step recovery, the narration fix, the shared/slimmed system prompt, and the red-build fix.
...
- **Awaiting CI:** `5a18bbf` — the orb is real 3D geometry (tilted rings, perspective projection, depth shading, additive blend) and the sprite artwork is deleted. **The open risk is framerate, not looks:** Reactor and Forge draw 5–6 rings x 24 depth-shaded chunks. If it stutters, cut `CHUNKS` in `Orb3DRenderer` before redesigning anything — it is a one-line change.
